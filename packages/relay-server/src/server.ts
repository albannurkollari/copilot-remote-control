...
export class RelayServer {
	...
	#getAllClients() {
		return [...this.#clients.discord.values(), ...this.#clients.vscode.values()];
	}
	...
	#handleSocketMessage(socket: WebSocket, data: RawData) {
		...
		if (!registeredClient) {
			this.#sendStatus(socket, 'error', 'unsupported_message', 'Clients must register before sending messages.');
			socket.close();
			return;
		}
		...
		switch (parsed.value.type) {
			...
			case 'relay_status':
			case 'pong':
				this.#sendStatus(socket, 'error', 'unsupported_message', `Message type ${parsed.value.type} is server-managed.`);
				return;
		}
	}
	...
	#registerClient(socket: WebSocket, message: RegisterMessage) {
		...
		const existingClient = this.#clients[message.clientRole].get(message.clientId);
	...
	#handlePrompt(client: RegisteredClient, message: CopilotPromptMessage) {
		...
		if (!vscodeClient) {
			this.#sendStatus(
				...
				{ requestId: message.requestId, clientId: message.clientId, targetClientRole: 'vscode' }
			);
			return;
		}
	...
	#handlePermissionRequest(client: RegisteredClient, message: PermissionRequestMessage) {
		if (client.role !== 'vscode') {
			this.#sendStatus(
				client.socket,
				'error',
	...
	#handlePermissionResponse(client: RegisteredClient, message: PermissionResponseMessage) {
		if (client.role !== 'discord') {
			this.#sendStatus(
				client.socket,
				'error',
		...
		if (!vscodeClient) {
			this.#sendStatus(
				...
				{ requestId: message.requestId, clientId: request.targetClientId, targetClientRole: 'vscode' }
			);
			this.#requests.delete(message.requestId);
			return;
		}
	...
	#handleSocketClose(socket: WebSocket) {
		...
		const pendingEntries = [...this.#requests.entries()].filter(([, request]) => {
	...
	#broadcastStatus(
		...
		message: string,
		details: { clientId?: string; requestId?: string; targetClientRole?: ClientRole } = {}
	) {
		const statusMessage = {
...
export const startRelayServer = async (options: RelayServerOptions = loadRelayServerOptions()) => {
