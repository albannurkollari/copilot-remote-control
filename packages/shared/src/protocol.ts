...
const hasOwn = <K extends string>(value: object, key: K): value is object & Record<K, unknown> => {
...
const isRelayStatusLevel = (value: unknown): value is RelayStatusLevel => {
	return isString(value) && RELAY_STATUS_LEVELS.includes(value as RelayStatusLevel);
};

const isPermissionAction = (value: unknown): value is PermissionAction => {
	return isString(value) && PERMISSION_ACTIONS.includes(value as PermissionAction);
};
...
export const parseRelayMessage = (value: unknown): ParseRelayMessageResult => {
	...
	switch (message.type) {
		case 'register':
			if (!isClientRole(message.clientRole) || !isNonEmptyString(message.clientId)) {
				return { ok: false, error: 'Register messages require clientRole and clientId.' };
			}
		...
		case 'relay_status':
			...
			if (hasOwn(message, 'requestId') && message.requestId !== undefined && !isNonEmptyString(message.requestId)) {
				return { ok: false, error: 'Relay status requestId must be a non-empty string.' };
			}

			if (hasOwn(message, 'clientId') && message.clientId !== undefined && !isNonEmptyString(message.clientId)) {
				return { ok: false, error: 'Relay status clientId must be a non-empty string.' };
			}
			...
			if (
				...
				return { ok: false, error: 'Relay status targetClientRole must be a valid client role.' };
			}
			...
			return {
				...
				value: {
					...
					requestId: isString(message.requestId) ? message.requestId : undefined,
		...
		case 'copilot_prompt':
			...
			return {
				...
				value: {
					...
					userDisplayName: isString(message.userDisplayName) ? message.userDisplayName : undefined,
					channelId: isString(message.channelId) ? message.channelId : undefined,
		...
		case 'copilot_stream':
			...
			if (hasOwn(message, 'delta') && message.delta !== undefined && !isString(message.delta)) {
				return { ok: false, error: 'Copilot stream delta must be a string.' };
			}

			if (hasOwn(message, 'error') && message.error !== undefined && !isString(message.error)) {
				return { ok: false, error: 'Copilot stream error must be a string.' };
			}

			if (hasOwn(message, 'modelId') && message.modelId !== undefined && !isString(message.modelId)) {
		...
		case 'permission_request':
			...
			if (hasOwn(message, 'details') && message.details !== undefined && !isString(message.details)) {
				return { ok: false, error: 'Permission request details must be a string.' };
			}

			if (hasOwn(message, 'command') && message.command !== undefined && !isString(message.command)) {
				return { ok: false, error: 'Permission request command must be a string.' };
			}
		...
		case 'permission_response':
			...
			if (hasOwn(message, 'reason') && message.reason !== undefined && !isString(message.reason)) {
				return { ok: false, error: 'Permission response reason must be a string.' };
			}
		...
		case 'pong':
			if (!isNonEmptyString(message.timestamp)) {
				return { ok: false, error: `${message.type} messages require a timestamp.` };
			}
		...
		default:
			return { ok: false, error: `Unsupported relay message type: ${message.type}` };
	}
};
