...
export interface PermissionRequester {
	(message: PermissionRequestMessage): Promise<{ approved: boolean; reason?: string }>;
}
...
const REMOTE_TOOLS: vscode.LanguageModelChatTool[] = [
	{
		...
		description: 'Request approval before running a terminal command in the local workspace.',
		inputSchema: {
			type: 'object',
			properties: {
				command: {
...
export class CopilotBridge {
	...
	async authorizeAccess() {
		...
		if (this.#context.languageModelAccessInformation.canSendRequest(model) === true) {
		...
		try {
			const response = await model.sendRequest(
				[vscode.LanguageModelChatMessage.User('Reply with exactly the word "authorized".')],
				{},
				tokenSource.token
			);
	...
	async runPrompt(message: CopilotPromptMessage, handlers: RunPromptHandlers) {
		...
		const tokenSource = new vscode.CancellationTokenSource();
		const conversation = [vscode.LanguageModelChatMessage.User(this.#renderPrompt(message))];

		try {
			const response = await model.sendRequest(conversation, { tools: REMOTE_TOOLS }, tokenSource.token);
			await this.#streamResponse(response, model, conversation, message, handlers, tokenSource.token);
		} catch (error) {
			throw new Error(this.#toUserFacingError(error));
		} finally {
			tokenSource.dispose();
		...
		for await (const part of response.stream) {
			...
			if (part instanceof vscode.LanguageModelToolCallPart) {
				...
				if (!approval.approved) {
					throw new Error(approval.reason ?? `Permission denied for ${part.name}.`);
				}
				...
				const followUp = await model.sendRequest(conversation, { tools: REMOTE_TOOLS }, token);
				await this.#streamResponse(followUp, model, conversation, prompt, handlers, token);
				return;
			}
		}
	}
	...
	async #selectModel() {
		...
		if (models.length === 0) {
			throw new Error('No GitHub Copilot chat model is available in this VS Code instance.');
		}
	...
	#toPermissionRequest(
		...
		const { action, command, details, title } = this.#describeToolCall(part.name, part.input);
	...
	#describeToolCall(name: string, input: object): {
