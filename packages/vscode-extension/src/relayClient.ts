...
export class VscodeRelayClient implements vscode.Disposable {
	...
	async connect() {
		...
		socket.on('open', () => {
			this.#log(`Connected to relay ${this.url}. Registering as ${this.clientId}.`);
			this.#sendRaw({
				type: 'register',
				clientRole: 'vscode',
	...
	async requestPermission(message: PermissionRequestMessage, timeoutMs = 120_000) {
		await this.waitForReady();

		const key = `${message.requestId}:${message.permissionId}`;
		const responsePromise = new Promise<PermissionResponseMessage>((resolve, reject) => {
		...
		return Promise.race([
			...
			delay(timeoutMs).then(() => {
				...
				throw new Error(`Timed out waiting for permission response after ${timeoutMs}ms.`);
			})
		]);
	}
	...
	async disconnect() {
		...
		for (const pending of this.#permissionResponses.values()) {
			pending.reject(new Error('Relay connection closed before a permission response arrived.'));
		}
	...
	async #handleMessage(raw: string) {
		...
		switch (parsed.value.type) {
			case 'register_ack':
				this.#isReady = true;
				this.#resolveReady?.();
				this.#log(`Relay registration acknowledged with connection ${parsed.value.connectionId}.`);
				return;
	...
	#handleDisconnect() {
		...
		this.#reconnectTimer = setTimeout(() => {
			...
			void this.connect().catch((error) => {
				this.#log(`Relay reconnect failed: ${error instanceof Error ? error.message : String(error)}`);
				this.#handleDisconnect();
			});
